...
(function($, window, document, undefined) {
    $.fn.socialfeed = function(_options) {
        ...
        SocialFeedPost.prototype = {
            render: function() {
                if (this.content['social_network'] == 'google')
                    this.content['social_network'] = 'google-plus';
        ...
        var Feed = {
                            ...
                            if (element.object.attachments) {
                                $.each(element.object.attachments, function() {
                                    ...
                                    if (this.fullImage) {
                                        ...
                                        if (this.objectType === 'video') {
                                            post.videoUrl = this.url;
                                            image = this.image.url;
                                        }
                                    }
                                    post.attachment = '<a href="' + post.videoUrl + '"><img class="attachment" src="' + image + '"/></a>';
                                });
                            }
                        }
            ...
            youtube: {
                ...
                getData: function(account) {
                    var request_url;
                    var channelId = account;
                    request_url = Feed.youtube.api + 'search?part=snippet,id&order=date' + '&channelId=' + channelId + '&key=' + options.youtube.access_token + '&maxResults=' + options.youtube.limit;
                    Utility.get_request(request_url, Feed.youtube.utility.getVideos);
                },
                utility: {
                    getVideos: function(json) {
                        if (json.items) {
                            $.each(json.items, function(i) {
                                var post = new SocialFeedPost('youtube', Feed.youtube.utility.unifyPostData(json.items[i]));
                                post.render();
                            });
                        }
                    },
                    unifyPostData: function(element) {
                        var post = {};
                        post.channelId = element.snippet.channelId;
                        post.id = element.id.videoId;
                        post.attachment = '';
                        post.description = element.snippet.description;
                        post.dt_create = moment(element.snippet.publishedAt);
                        post.author_link = 'https://www.youtube.com/channel/' + post.channelId;
                        // post.author_picture = Feed.youtube.profileImg;
                        post.author_name = element.snippet.channelTitle;

                        if (options.show_media === true) {
                            if (element.snippet.thumbnails) {
                                $.each(element.snippet.thumbnails, function() {
                                    var image = '';
                                    image = this.url;

                                    post.attachment = '<iframe src="https://www.youtube.com/embed/' + post.id + '" frameborder="0" allowfullscreen></iframe>';
                                });
                            }
                        }
                        post.message = element.snippet.title;
                        post.link = 'https://www.youtube.com/watch?v=' + post.id;
                        channel_url = Feed.youtube.api + 'channels?part=snippet&id=' + post.channelId + '&key=' + options.youtube.access_token;
                        $.ajax({
                            url: channel_url,
                            success: function(json) {
                                post.author_picture = json.items[0].snippet.thumbnails.high.url;
                            },
                            dataType: 'json',
                            async: false
                        });
